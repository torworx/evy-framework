...
import evyframework.common.io.support.EncodedResource;
...
import evyframework.container.factory.support.AbstractFactoryDefinitionParser;
import evyframework.container.factory.support.ParserException;

public class ScriptFactoryDefinitionParser extends AbstractFactoryDefinitionParser {

	private final ThreadLocal<Set<EncodedResource>> resourcesCurrentlyBeingLoaded = new ThreadLocal<Set<EncodedResource>>();

	public ScriptFactoryDefinitionParser(FactoryDefinitionRegistry registry) {
		super(registry);
	}

	@Override
	public int loadFactoryDefinitions(Resource resource) throws FactoryDefinitionStoreException {
		return loadFactoryDefinitions(new EncodedResource(resource));
	}

	/**
	 * Load factory definitions from the specified config file.
	 * 
	 * @param encodedResource
	 *            the resource descriptor for the config file, allowing to specify
	 *            an encoding to use for parsing the file
	 * @return the number of factory definitions found
	 * @throws FactoryDefinitionStoreException
	 *             in case of loading or parsing errors
	 */
	public int loadFactoryDefinitions(EncodedResource encodedResource) throws FactoryDefinitionStoreException {
		Assert.notNull(encodedResource, "EncodedResource must not be null");
		if (logger.isInfoEnabled()) {
			logger.info("Loading config factory definitions from " + encodedResource.getResource());
		}
		...
		if (!currentResources.add(encodedResource)) {
			throw new FactoryDefinitionStoreException("Detected cyclic loading of " + encodedResource
		...
		try {
			...
			try {
				...
				return doLoadFactoryDefinitions(reader, encodedResource.getResource());
			} finally {
				inputStream.close();
			}
		} catch (IOException ex) {
			throw new FactoryDefinitionStoreException("IOException parsing config document from "
	...
	/**
	 * Actually load factory definitions from the specified config file.
	 * @param reader the Reader to read from
	 * @param resource the resource descriptor for the config file
	 * @return the number of factory definitions found
	 * @throws FactoryDefinitionStoreException in case of loading or parsing errors
	 */
	protected int doLoadFactoryDefinitions(Reader reader, Resource resource)
			throws FactoryDefinitionStoreException {
		int countBefore = getRegistry().getFactoryDefinitionCount();
		ParserInput parserInput = new ParserInput(reader);
		FactoryDefinition definition = parseFactory(parserInput);
		while (definition != null) {
			getRegistry().registerFactoryDefinition(definition.getName(), definition);
			definition = parseFactory(parserInput);
		}
		return getRegistry().getFactoryDefinitionCount() - countBefore;
	}
	

	protected FactoryDefinition parseFactory(ParserInput input) {
		input.factoryStart();
		if (input.lookAhead() == null)
			return null;
		FactoryDefinition definition = null;
		String id = parseGlobalFactoryName(input);
		
		if (id.startsWith(Token.AMPERSAND.toString())) {
			definition = new FactoryDefinition();
			definition.setName(FactoryUtil.genAutoName());
			definition.setFactoryType(FactoryDefinition.FACTORY_UPDATE_FACTORY);
			definition.setIdentifier(id.substring(1));
		} else {
			...
			try {
				...
				throw new ParserException("ScriptFactoryParser", "ERROR_PARSING_FACTORY", "Error in factory definition "
						+ id, e);
			}
		...
		while (hasPhases) {
			...
			try {
				...
				e.addInfo("ScriptFactoryParser", "ERROR_PARSING_FACTORY_PHASES", "Error in factory definition " + id
						+ " in phases definition");
				throw e;
			} catch (Exception e) {
				throw new ParserException("ScriptFactoryParser", "ERROR_PARSING_FACTORY_PHASES",
						"Error in factory definition " + id + " in phases definition");
			}
	...
	/**
	 * Parses a comma separated list of factory chains. It will not work with
	 * semi-colon separated lists. Only comma.
	 * 
	 * @param input
	 *            The ParserInput to parse the factory chain list from.
	 * @return A list of factory definitions coresponding to the factory chain
	 *         lists parsed.
	 */
	protected List<FactoryDefinition> parseFactoryChainList(ParserInput input) {
		// TODO optimize out to a constant EMPTY_LIST for performance
		List<FactoryDefinition> factoryChainList = new ArrayList<FactoryDefinition>();
		...
		while (!isEndOfFactoryChainList(nextToken)) {
			...
			input.mark();
			nextToken = input.nextToken();
			if (Token.COMMA.equals(nextToken))
				input.clearMark();
			else if (isEndOfFactoryChainList(nextToken))
				input.backtrack();
			else
	...
	protected FactoryDefinition parseFactoryChain(ParserInput input) {
		...
		delimiter = input.lookAhead();
		delimiter = input.lookAhead();
		if (Token.SEMI_COLON.equals(delimiter))
			return definition;
	...
	private FactoryDefinition parseTailFactoryChain(ParserInput input, FactoryDefinition definition) {
		...
		Token delimiter = input.nextToken();
		if (delimiter == null)
			throw new ParserException("ScriptFactoryParser", "PARSE_TAIL_FACTORY_CHAIN",
	...
	protected FactoryDefinition parseHeadFactory(ParserInput input, FactoryDefinition definition) {
		...
		if (headFactory == null)
			headFactory = new FactoryDefinition();
		...
		if (nextToken.startsWith(Token.HASH)) {
			...
			} else { // parse property placeholder like ${var} as value factory
				input.assertNextToken(Token.CURLY_LEFT);
				StringBuffer sb = new StringBuffer(nextToken.toString()).append(Token.CURLY_LEFT);
				nextToken = input.nextToken();
				while (nextToken != null && !nextToken.equals(Token.CURLY_RIGHT)) {
					sb.append(nextToken.toString());
					nextToken = input.nextToken();
				}
				sb.append(Token.CURLY_RIGHT);
				headFactory.setFactoryType(FactoryDefinition.VALUE_FACTORY);
				headFactory.setIdentifier(sb.toString());
			}
			input.clearMark();
			return headFactory;
		...
		if (className != null) {
			...
			if (Token.PARENTHESIS_LEFT.equals(delimiter)) {
				...
				// factory chain is finished already. It was just a class,
				// nothing else. Set as value factory.
				headFactory.setFactoryType(FactoryDefinition.VALUE_FACTORY);
				headFactory.setIdentifier(className);
				input.backtrack();
			}
		} else {
			nextToken = input.nextToken(); // re-take the token. it was pushed
											// back when checking for class name
											// earlier.
	...
	protected FactoryDefinition parseMethodOrFieldHeadFactory(ParserInput input, FactoryDefinition headFactory,
			String className) {
		Token identifier = input.nextToken();
		input.mark();
		Token nextDelimiter = input.nextToken();
		if (Token.PARENTHESIS_LEFT.equals(nextDelimiter)) {
			...
			throw new ParserException("ScriptFactoryParser", "PARSE_METHOD_OR_FIELD_FACTORY",
					"Expected either a method or field, but found: " + identifier + nextDelimiter);
		}
		return headFactory;
	}
	...
	protected String parseClass(ParserInput input) {
		...
		while (classInstance == null) {
			...
			if (!".".equals(delimiter)) {
				input.backtrack(); // backtrack to beginning of
									// "assumed class name".
				return null;
			}
	...
	protected Token parseMode(ParserInput input) {
		...
		Token mode = null;
		Token nextToken = input.nextToken();
		if (Token.NEW_INSTANCE.equals(nextToken) || Token.FLYWEIGHT.equals(nextToken)
				|| Token.THREAD_SINGLETON.equals(nextToken) || Token.LOCALIZED.equals(nextToken)) {
			mode = nextToken;
			input.clearMark();
			...
			if (Token.SEMI_COLON.equals(tokenAfterMode)) {
				// The factory definition was 'mode = 1 ;' which means 1 is not
				// the instantiation mode,
				// but the value of a value factory, using property style
				// configuration.
				// Therefore we backtrack the 1 parsed as instantiation mode,
				// and let it be parsed
				// as a factory value. Note however, that instantiation mode of
				// property style factories is as
				// singleton!
		...
		// throw new ParserException("Factory mode was: " + mode +
		// ". Mode must be either '*', '+', '1', 'n' or 'n,m'");
	}

	/**
	 * Global factory names are allowed to contain a single period / dot, just
	 * like a normal URI can have. E.g. <br/>
	 * <br/>
	 * 
	 ...
	 * 
	 * @param input
	 *            The ParserInput instance that is currently being parsed a
	 *            factory from
	...
	// ==================================================================================
	// General Utility Methods ... perhaps move to ParserInput or other utility
	// class???
	// ==================================================================================
	...
	/*
	 * private boolean isInteger(Token token){ return token.isInteger(); }
	 * 
	 * private boolean isNumber(Token token) { return token.isNumber(); }
	 */
	...
	private boolean isStringValue(Token token) {
		...
		if (token.startsWith(Token.QUOTE) || token.endsWith(Token.QUOTE)) {
			throw new ParserException("ScriptFactoryParser", "IS_STRING_VALUE",
	...
	private boolean isEndOfFactoryChain(Token delimiter) {
		if (Token.CURLY_LEFT.equals(delimiter))
			return true;
		if (Token.CURLY_RIGHT.equals(delimiter))
			return true;
		if (Token.COMMA.equals(delimiter))
			return true;
		if (Token.SEMI_COLON.equals(delimiter))
			return true;
		if (Token.PARENTHESIS_RIGHT.equals(delimiter))
			return true;
		if (Token.SQUARE_RIGHT.equals(delimiter))
			return true;
		if (Token.GREATER_THAN.equals(delimiter))
			return true;
		if (Token.COLON.equals(delimiter))
			return true;
		return false;
	}
