...
public class FactoryDefinition {
	...
	/*
	 * todo Pool instances of FactoryDefinition like done with Token, to limit the number of FactoryDefinition instances
	 * created during parsing.
	 */

	/*
	 * These constants are only used while building the factories. After that they are forgotten. They are strings to
	 * ease debugging of the parser/builder.
	 */
	public static final String FACTORY_UPDATE_FACTORY = "Factory Update Factory";
	...
	public boolean isNewInstance() {
		if (getName() == null)
			return true; // anonymous factories are always new instance factories.
		return "*".equals(getMode());
	}

	public boolean isSingleton() {
		if (isInputParameterFactory())
			return false;
	...
	public boolean isFactoryUpdateFactory() {
		return this.factoryType == FACTORY_UPDATE_FACTORY;
	}
...
//	public boolean isFactoryReferenceFactory() {
//		return this.factoryType == FACTORY_REFERENCE_FACTORY;
//	}
	...
	public Map<String, Integer> getLocalFactoryNameIndexMap() {
		if (getRootAncestor() == this)
			return this.localFactoryNameIndexMap;
	...
	public Map<String, Class<?>> getLocalFactoryNameTypeMap() {
		if (getRootAncestor() == this)
			return localFactoryNameTypeMap;
	...
	public int getLocalProductIndex() {
		if (getName() == null && getIdentifier() == null)
			throw new IllegalStateException(
					"Only a factory with a name or local product reference has a local product index");

		Map<String, Integer> nameIndexMap = getLocalFactoryNameIndexMap();

		if (getName() != null) {
			if (nameIndexMap.get(getName()) == null)
				throw new ParserException("FactoryDefinition", "UNKNOWN_FACTORY", "Uknown factory: " + getName());
			return nameIndexMap.get(getName());
		} else {
			if (nameIndexMap.get(getIdentifier()) == null)
				throw new ParserException("FactoryDefinition", "UNKNOWN_FACTORY", "Uknown factory: " + getIdentifier());
	...
	public String getResourceDescription() {
		if (this.resourceDescription == null && (getRoot() == null || getRoot() == this)) {
			return null;
		}
		return this.resourceDescription != null ? this.resourceDescription : getRoot().getResourceDescription();
	}
	...
	private void mapLocalFactoryNamesToIndex(Collection<FactoryDefinition> definitions,
